...
def main():
    ...
    #1.一般需要将criterion设置为"gini"或"entropy",它们计算类内不纯度,适合多类别分类
    #2."gini"稍微简单一些,计算样本分错类的概率。"entropy"计算信息熵,稍微复杂一点。
    rf_model.fit(X_train, y_train)
    logging.info("finish training %d sec" % (time.time() - start_time))
    ############### 训练效果 ###############
    rf_Y = rf_model.predict(X_train)

    f_max, p_max, r_max, t_max, predictions_max,_,_ = compute_performance(rf_Y
                                                                      , y_train
                                                                      , gos=train_gos
                                                                      , all_functions=args["functions"]
                                                                      , GO_ID=args["Go_id"], func_set=args["func_set"]
                                                                      , go=args["go"])
    roc_auc = compute_roc(rf_Y, y_train, args)  # torch.Size([3141, 589]),(31530, 1) 多维数据的降维函数
    ...
    f_max, p_max, r_max, t_max, predictions_max,recall_list, predictions_list = compute_performance(rf_pred
                                                                      , y_test
                                                                      , gos=test_gos
                                                                      , all_functions=args["functions"]
                                                                      , GO_ID=args["Go_id"], func_set=args["func_set"]
                                                                      , go=args["go"])
    roc_auc = compute_roc(rf_pred, y_test,args)  # torch.Size([3141, 589]),(31530, 1) 多维数据的降维函数
    #print("the loss of testing is %f." % (test_Loss))
    ...
    # 1. 创建文件对象
    f = open('./res/res_rf_'+args["function"]+'.csv', 'w', encoding='utf-8', newline="")
    # 2. 基于文件对象构建 csv写入对象
