...
def main():
    ...
    n_components = 2
    umap_embedding = umap.UMAP(n_neighbors=20, min_dist=0.1, n_components=n_components,n_epochs=10)
    ...
    # embedding = np.reshape(embedding, newshape=(-1, args["nb_classes"], n_components))
    ...
    maxlabel_4 = np.argmax(sumLabel)
    sumLabel[maxlabel_4] = 0

    MaxLabel = [maxlabel_1, maxlabel_2, maxlabel_3] #最多的标签
    print(functions[maxlabel_1], functions[maxlabel_2], functions[maxlabel_3],functions[maxlabel_4])
    subset_label = [functions[maxlabel_1], functions[maxlabel_2], functions[maxlabel_3],functions[maxlabel_4]]
    subset_color = ["b","r","k","y"]
    ############# 改变维度 ################
    # all_out1 = np.reshape(all_out[:,MaxLabel], newshape=(-1, 32))
    # Y = np.reshape(test_y[:,MaxLabel], newshape=(-1, 1))
    # embedding = umap_embedding.fit_transform(X=all_out1 , y=Y)  # (100, 2),输入特征X，输出降维后的特征embedding
    ...
    for i in range(len(MaxLabel)):
        ...
        all_out1 = np.reshape(all_out[:, label,:], newshape=(-1, 32))
        Y = np.reshape(test_y[:, label], newshape=(-1, 1))
        embedding = umap_embedding.fit_transform(X=all_out1, y=Y)  # (100, 2),输入特征X，输出降维后的特征embedding
        embedding = np.reshape(embedding, newshape=(-1,  n_components))
        plt.scatter(embedding[indices,0], embedding[indices,1], marker='.', alpha=0.5,label = subset_label[i])
        indices = np.where(test_y[:, label] != 1.0)
        plt.scatter(embedding[indices, 0], embedding[indices, 1], marker='.', alpha=0.2, label="Other")
        ...
        plt.title('UMAP Projection of Data with %s '% functions[label],fontsize=17)  # 多类
        plt.xlabel('UMAP 1',fontsize=19)
        plt.ylabel('UMAP 2',fontsize=19)
        plt.yticks(size=19)
        plt.xticks(size=19)
        plt.legend()
        plt.savefig('./image/' + args["function"] + '_GO'+functions[label][3:]+'_UMAP.eps',bbox_inches = 'tight')  # eps文件，用于LaTeX
        #plt.savefig('./image/' + args["function"] + '_GO' + functions[label][3:] + '_UMAP.eps')
        plt.show()
